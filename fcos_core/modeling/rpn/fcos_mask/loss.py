...
class FCOSMaskLossComputation(object):
    """
    ...
    def compute_targets_for_locations(self, locations, feature_size, targets, object_sizes_of_interest):
        ...
        for im_i in range(len(targets)):
            ...
            for size in feature_size:
                with torch.no_grad():
                    resized_masks_per_im = F.adaptive_avg_pool2d(Variable(masks_per_im.float()), size).data
                masks.append(resized_masks_per_im.reshape(len(bboxes), -1).permute(1, 0))
...
def make_fcos_mask_loss_evaluator(cfg):
    loss_evaluator = FCOSMaskLossComputation(cfg)
