...
from .inference import make_fcos_mask_postprocessor
from .loss import make_fcos_mask_loss_evaluator

from fcos_core.layers import Scale


class FCOSMaskHead(torch.nn.Module):
...
class FCOSMaskModule(torch.nn.Module):
    """
    ...
    def __init__(self, cfg, in_channels):
        super(FCOSMaskModule, self).__init__()

        head = FCOSMaskHead(cfg, in_channels)

        box_selector_test = make_fcos_mask_postprocessor(cfg)

        loss_evaluator = make_fcos_mask_loss_evaluator(cfg)
        self.head = head
...
def build_fcos_mask(cfg, in_channels):
    return FCOSMaskModule(cfg, in_channels)
