...
def main():
    ...
    parser.add_argument(
        "--config-file",
        default="configs/fcos_mask/fcos_mask_R_50_FPN_1x.yaml",
        metavar="FILE",
        help="path to config file",
    )
    parser.add_argument(
        "--weights",
        default="models/fcos_mask_test3/model_final.pth",
        metavar="FILE",
    ...
    for im_name in demo_im_names:
        ...
        boxes, box_masks = coco_demo.my_run(img)
        box_masks_sig = []
        for mask in box_masks:
            box_masks_sig.append(torch.sigmoid(mask))
